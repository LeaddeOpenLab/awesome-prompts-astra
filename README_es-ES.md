# Astra · Biblioteca de prompts · Leadde.ai

[![English](https://img.shields.io/badge/English-lightgrey)](README.md) [![简体中文](https://img.shields.io/badge/%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87-lightgrey)](README_zh.md) [![繁體中文](https://img.shields.io/badge/%E7%B9%81%E9%AB%94%E4%B8%AD%E6%96%87-lightgrey)](README_zh-TW.md) [![日本語](https://img.shields.io/badge/%E6%97%A5%E6%9C%AC%E8%AA%9E-lightgrey)](README_ja-JP.md) [![한국어](https://img.shields.io/badge/%ED%95%9C%EA%B5%AD%EC%96%B4-lightgrey)](README_ko-KR.md) [![ไทย](https://img.shields.io/badge/%E0%B9%84%E0%B8%97%E0%B8%A2-lightgrey)](README_th-TH.md) [![Tiếng Việt](https://img.shields.io/badge/Ti%E1%BA%BFng%20Vi%E1%BB%87t-lightgrey)](README_vi-VN.md) [![हिन्दी](https://img.shields.io/badge/%E0%A4%B9%E0%A4%BF%E0%A4%A8%E0%A5%8D%E0%A4%A6%E0%A5%80-lightgrey)](README_hi-IN.md) [![Español](https://img.shields.io/badge/Espa%C3%B1ol-brightgreen)](README_es-ES.md) [![Español (Latinoamérica)](https://img.shields.io/badge/Espa%C3%B1ol%20(Latinoam%C3%A9rica)-lightgrey)](README_es-419.md) [![Deutsch](https://img.shields.io/badge/Deutsch-lightgrey)](README_de-DE.md) [![Français](https://img.shields.io/badge/Fran%C3%A7ais-lightgrey)](README_fr-FR.md) [![Italiano](https://img.shields.io/badge/Italiano-lightgrey)](README_it-IT.md) [![Português (Brasil)](https://img.shields.io/badge/Portugu%C3%AAs%20(Brasil)-lightgrey)](README_pt-BR.md) [![Português](https://img.shields.io/badge/Portugu%C3%AAs-lightgrey)](README_pt-PT.md) [![Türkçe](https://img.shields.io/badge/T%C3%BCrk%C3%A7e-lightgrey)](README_tr-TR.md)

**Best for:** Astra prompts for 3D scenes, Blender workflows, game assets, and agent-led visual production.

For PDF, PPT, SOP, training, and multilingual video workflows:
[Awesome Document-to-Video](https://github.com/LeaddeOpenLab/awesome-document-to-video)

> **Prompts de calidad seleccionados cada día**

Descubre prompts completos para crear imágenes, vídeos y 3D con IA. Explora por estilo, consulta versiones multilingües y encuentra a los autores originales.

[Explora Leadde.ai →](https://leadde.ai/?utm_source=github&utm_medium=readme&utm_campaign=prompt-library&utm_content=astra)

## Conoce Leadde.ai

Leadde.ai ayuda a los equipos a convertir documentos, diapositivas y texto en vídeos empresariales con IA para formación, incorporación y marketing.

Marca este repositorio con una estrella para seguir nuestra selección diaria e inspirarte con nuevas ideas.

**14** Prompts · Última incorporación: **2026-09-23**

<a name="catalog"></a>

## Explorar por categoría

[Cine / Fotograma de película](#category-cinematic-film-still) · [Ilustración](#category-illustration) · [Cómic / Novela gráfica](#category-comic-graphic-novel) · [Render 3D](#category-3d-render)

<a name="all-prompts"></a>

<a name="category-cinematic-film-still"></a>

## Cine / Fotograma de película

<a name="prompt-2097777121452347846"></a>

### Crea una imagen de guion gráfico de 24 fotogramas que contenga 24 capturas de pantalla de una posible película reboot de Seinfeld.

Autor：[@VK10920178](https://x.com/VK10920178) · [Publicación original](https://x.com/VK10920178/status/2097777121452347846)

Cómic / Guion gráfico · Cine / Fotograma de película · Publicado

**Resumen:** Crea una imagen de guion gráfico de 24 fotogramas que contenga 24 capturas de pantalla de una posible película reboot de Seinfeld.

<img src="images/2097777121452347846-1.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Crea una imagen de guion gráfico de 24 fotogramas que contenga 24 capturas de pantalla de una posible película reboot de Seinfeld.
```

[↑ Volver a categorías](#catalog)

---

<a name="category-illustration"></a>

## Ilustración

<a name="prompt-2096842636074312057"></a>

### Traducción en curso

Autor：[@Chengzilhy](https://x.com/Chengzilhy) · [Publicación original](https://x.com/Chengzilhy/status/2096842636074312057)

Perfil / Avatar · Diseño de aplicaciones / web · Fotografía · Ilustración · Personaje · Publicado

Publicación original：[@Chengzilhy](https://x.com/Chengzilhy) · [Publicación original](https://x.com/Chengzilhy/status/2096456212246364322)

**Resumen:** Traducción en curso

<img src="covers/2096842636074312057.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Traducción en curso
```

[↑ Volver a categorías](#catalog)

---

<a name="category-comic-graphic-novel"></a>

## Cómic / Novela gráfica

<a name="prompt-2097718063663858058"></a>

### Anuncio de chicle escolar de 30 segundos: un estudiante tímido al que acosan mastica chicle “RAZOR” para recuperar la confianza, exhibe trucos de skate para superar al abusón e impresiona a una pelirroja, cerrando con efectos de cómic en amarillo y negro y el eslogan del producto.

Autor：[@higgsfield\_ai](https://x.com/higgsfield_ai) · [Publicación original](https://x.com/higgsfield_ai/status/2097718063663858058)

Cómic / Novela gráfica · Personaje · Producto · Publicado

**Resumen:** Anuncio de chicle escolar de 30 segundos: un estudiante tímido al que acosan mastica chicle “RAZOR” para recuperar la confianza, exhibe trucos de skate para superar al abusón e impresiona a una pelirroja, cerrando con efectos de cómic en amarillo y negro y el eslogan del producto.

<img src="covers/2097718063663858058.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Crea un anuncio cinematográfico de chicle de 30 segundos ambientado en el patio de una escuela. Un estudiante tímido con camisa morada es acosado por atletas que llevan chaquetas universitarias verdes. Un amigo le da un chicle “RAZOR” en un paquete amarillo. Lo mastica, gana confianza, realiza trucos increíbles con el monopatín, supera al abusón e impresiona a una chica pelirroja. Utiliza efectos visuales realistas, movimientos de cámara rápidos, humor travieso y efectos de cómic en amarillo y negro. Termina con el paquete de chicles y el eslogan.
```

[↑ Volver a categorías](#catalog)

---

<a name="category-3d-render"></a>

## Render 3D

<a name="prompt-2102565403109085669"></a>

### Traducción en curso

Autor：[@dotey](https://x.com/dotey) · [Publicación original](https://x.com/dotey/status/2102565403109085669)

Diseño de aplicaciones / web · Fotografía · Render 3D · Paisaje / Naturaleza · Publicado

**Resumen:** Traducción en curso

<img src="images/2102565403109085669-1.jpg" alt="Imagen 1" width="480" />

<img src="images/2102565403109085669-2.jpg" alt="Imagen 2" width="480" />

<img src="images/2102565403109085669-3.jpg" alt="Imagen 3" width="480" />

<img src="images/2102565403109085669-4.jpg" alt="Imagen 4" width="480" />

**Prompt**

```text
Traducción en curso
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2102091619973427380"></a>

### Construcción de una fábrica de clasificación de juguetes en Three.js, que abarca el transporte por cinta transportadora, la recogida con brazo robótico y la clasificación por colores en cajas.

Autor：[@yrzhe\_top](https://x.com/yrzhe_top) · [Publicación original](https://x.com/yrzhe_top/status/2102091619973427380)

Render 3D · Publicado

**Resumen:** Construcción de una fábrica de clasificación de juguetes en Three.js, que abarca el transporte por cinta transportadora, la recogida con brazo robótico y la clasificación por colores en cajas.

<img src="covers/2102091619973427380.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Crea una fábrica de clasificación de juguetes con Three.js. Una cinta transportadora entrega los juguetes, un brazo robótico los recoge y los coloca en cajas según su color.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2100136611166007364"></a>

### Prompt que indica a una IA reconstruir una escena 3D interactiva con movimientos articulados a partir de una foto de una habitación.

Autor：[@walterzhu8](https://x.com/walterzhu8) · [Publicación original](https://x.com/walterzhu8/status/2100136611166007364)

Fotografía · Render 3D · Publicado

**Resumen:** Prompt que indica a una IA reconstruir una escena 3D interactiva con movimientos articulados a partir de una foto de una habitación.

<img src="covers/2100136611166007364.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
A partir de la foto de la habitación que he proporcionado, usa Blender MCP para construir una escena 3D interactiva y renderízala en un vídeo de demostración. Incluye movimiento de objetos articulados (bisagras, puertas, cajones) y usa movimientos de cámara adecuados para mostrar estos efectos.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2100028214659932570"></a>

### Generación guiada por prompts de una página interactiva 3D en Three.js de una habitación en miniatura, con cambio de esquema de color e interacción básica de perspectiva.

Autor：[@wangdefou](https://x.com/wangdefou) · [Publicación original](https://x.com/wangdefou/status/2100028214659932570)

Fotografía · Render 3D · Publicado

**Resumen:** Generación guiada por prompts de una página interactiva 3D en Three.js de una habitación en miniatura, con cambio de esquema de color e interacción básica de perspectiva.

<img src="covers/2100028214659932570.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Usa frontend-design para crear una página web con Three.js basada en la imagen de la habitación y el diseño verificado.
El estilo debe ser una habitación en miniatura cálida, en tonos blanco roto y madera, con un panel de control conciso en chino.
Utiliza geometrías simples para modelar la cama, el escritorio, el armario, las puertas y las ventanas, sin depender de modelos 3D externos.
Implementa únicamente rotación por arrastre, zoom con la rueda del ratón, cambio entre dos combinaciones de colores y reinicio de la vista.
Mantén el diseño confirmado; enumera como suposiciones cualquier altura o material no especificado.
Una vez creada, inicia la página en local para comprobar las interacciones y conserva la primera versión.
Si surge algún problema, corrígelo a partir de capturas de pantalla reales o mensajes de error.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2099588840419651890"></a>

### Prompt para generar una escena 3D de pasillo de hotel.

Autor：[@MyWestLord](https://x.com/MyWestLord) · [Publicación original](https://x.com/MyWestLord/status/2099588840419651890)

Render 3D · Publicado

Publicación original：[@MyWestLord](https://x.com/MyWestLord) · [Publicación original](https://x.com/MyWestLord/status/2098109848118280487)

**Resumen:** Prompt para generar una escena 3D de pasillo de hotel.

<img src="covers/2099588840419651890.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
crear escena de pasillo de hotel
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2097864183773618361"></a>

### Simulación de vuelo de recolección en prado de flores silvestres desde la perspectiva de una abeja e interfaz de fijación de objetivos.

Autor：[@prompts\_ig](https://x.com/prompts_ig) · [Publicación original](https://x.com/prompts_ig/status/2097864183773618361)

Diseño de aplicaciones / web · Render 3D · Publicado

**Resumen:** Simulación de vuelo de recolección en prado de flores silvestres desde la perspectiva de una abeja e interfaz de fijación de objetivos.

<img src="covers/2097864183773618361.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Sigue una ruta simulada de recolección de polen a través de un prado de flores silvestres, con una superposición de búsqueda que resalta una flor a medida que la abeja se acerca.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2097817422485160261"></a>

### Instrucciones para modelar una silla de oficina en Maya, configurar texturas y renderizar un plato giratorio con Arnold

Autor：[@yuukixx\_vrc](https://x.com/yuukixx_vrc) · [Publicación original](https://x.com/yuukixx_vrc/status/2097817422485160261)

Render 3D · Publicado

**Resumen:** Instrucciones para modelar una silla de oficina en Maya, configurar texturas y renderizar un plato giratorio con Arnold

<img src="covers/2097817422485160261.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Conéctate al commandPort de Maya, modela una silla de oficina común, configura texturas fotorrealistas y renderiza un plato giratorio (turntable) con Arnold
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2097743367484621269"></a>

### Instrucción para generar un modelo de terreno 3D tridimensional de un pequeño pueblo isleño haciendo referencia a la imagen adjunta.

Autor：[@TaroKichijo](https://x.com/TaroKichijo) · [Publicación original](https://x.com/TaroKichijo/status/2097743367484621269)

Render 3D · Publicado

**Resumen:** Instrucción para generar un modelo de terreno 3D tridimensional de un pequeño pueblo isleño haciendo referencia a la imagen adjunta.

<img src="covers/2097743367484621269.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Usa img2threejs/img2threejs para crear el terreno tridimensional de un pequeño pueblo en esta isla. Usa la imagen adjunta como imagen de referencia.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2097681593246613583"></a>

### Villa moderna, piscina infinita, vistas al mar.

Autor：[@robinstetic](https://x.com/robinstetic) · [Publicación original](https://x.com/robinstetic/status/2097681593246613583)

Render 3D · Arquitectura / Interiorismo · Paisaje / Naturaleza · Publicado

**Resumen:** Villa moderna, piscina infinita, vistas al mar.

<img src="covers/2097681593246613583.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Villa moderna, piscina infinita, vistas al mar.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2097620442236555693"></a>

### Prompt para crear una escena de ensamblaje de un traje de hierro en Blender.

Autor：[@chatgpt\_liberty](https://x.com/chatgpt_liberty) · [Publicación original](https://x.com/chatgpt_liberty/status/2097620442236555693)

Render 3D · Publicado

**Resumen:** Prompt para crear una escena de ensamblaje de un traje de hierro en Blender.

<img src="covers/2097620442236555693.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Crea una escena de ensamblaje y puesta de un traje de hierro en Blender.
```

[↑ Volver a categorías](#catalog)

---

<a name="prompt-2097575421944738252"></a>

### Modelo 3D del exterior de la Tower of Terror versión de Tokio

Autor：[@satoh\_sama4](https://x.com/satoh_sama4) · [Publicación original](https://x.com/satoh_sama4/status/2097575421944738252)

Render 3D · Arquitectura / Interiorismo · Publicado

**Resumen:** Modelo 3D del exterior de la Tower of Terror versión de Tokio

<img src="covers/2097575421944738252.jpg" alt="Imagen 1" width="480" />

**Prompt**

```text
Crea un modelo 3D del exterior de la Tower of Terror versión de Tokio
```

[↑ Volver a categorías](#catalog)

---

[Explora Leadde.ai →](https://leadde.ai/?utm_source=github&utm_medium=readme&utm_campaign=prompt-library&utm_content=astra)
